...
var toString$9 = uncurryThis$m(1.0.toString);
...
var isSymbol$2 = USE_SYMBOL_AS_UID ? function (it) {
  ...
  return isCallable$k($Symbol) && isPrototypeOf$2($Symbol.prototype, $Object$2(it));
};
...
objectGetOwnPropertyDescriptor.f = DESCRIPTORS$7 ? $getOwnPropertyDescriptor : function getOwnPropertyDescriptor(O, P) {
  ...
  if (hasOwn$7(O, P)) return createPropertyDescriptor$3(!call$e(propertyIsEnumerableModule$1.f, O, P), O[P]);
};
...
$$e({ global: true, constructor: true, forced: !USE_NATIVE_URL }, {
...
const config$3 = {
  IAM_API_BASE_URL: 'http://localhost:8080/v2',
  BACKEND_BASE_URL: 'http://localhost:8090/v2'
};

const config$2 = {
  IAM_API_BASE_URL: 'https://api.staging.cencox.xyz/yumi-iam/api/v2',
  BACKEND_BASE_URL: 'https://api.staging.cencox.xyz/yumi-iam/bff/v2'
};

const config$1 = {
  IAM_API_BASE_URL: 'https://api.cencosudx.xyz/yumi-iam/api/v2',
  BACKEND_BASE_URL: 'https://api.cencosudx.xyz/yumi-iam/bff/v2'
};
...
(function (Providers) {
  Providers["txd"] = "txd";
  Providers["microsoft"] = "microsoft_saml";
  Providers["google"] = "google_gmail";
})(Providers || (Providers = {}));

const CLIENT_ID = 'arcus';
function toLoginUrl(provider) {
  const baseUrl = `${config.getValue('IAM_API_BASE_URL')}/oauth2`;
  const params = new URLSearchParams({
    response_type: 'token',
    prompt: 'login',
    redirect_uri: `${baseUrl}/callback`
  });
  const url = `${baseUrl}/${CLIENT_ID}/authorize/${provider}?${params}`;
  return url;
}
const Login = props => {
  const onAuthenticatedHandler = authData => __awaiter(void 0, void 0, void 0, function* () {
    SDK.Lib.IAM.authenticate(authData);
    props.onAuthenticated();
  });
  const onLoginHandler = provider => {
    const loginUrl = toLoginUrl(provider);
  ...
  if (props.children) {
    return jsx(props.children, {
      ...
      providers: loginButtons.map(btn => btn.provider)
    });
  }
...
class Approval$1 extends SDK.Lib.RESTClient {
...
var Approval$2 = (() => new Approval$1({
  baseURL: config.getValue('BACKEND_BASE_URL')
}));

class User extends SDK.Lib.RESTClient {
...
class Role extends SDK.Lib.RESTClient {
...
class Section extends SDK.Lib.RESTClient {
  getAll(country, flag, store) {
    return __awaiter(this, void 0, void 0, function* () {
...
const List = () => {
  ...
  const onBootHandler = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
      setViewMode('PENDING');
      const newResult = yield Approval$2().getAll(context.country, context.flag, context.store);
      setResult(newResult);
...
const thereAreChanges$1 = (country, flag, store, approval, draft) => {
  ...
  thereAreChanges = !!_.xor(userRoles, draftRoles).length;
...
const Update = () => {
  ...
  const onBootHandler = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
      setViewMode('PENDING');
      const [task, roles, sections] = yield Promise.all([Approval$2().get(context.country, context.flag, context.store, taskId), Role$1().getAll(context.country, context.flag, context.store), Section$1().getAll(context.country, context.flag, context.store)]);
      ...
      // if there are sub-options in any section, then
      // we are working with sectors, the backend control this
    ...
    setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
      if (action === 'confirm') {
        try {
          ...
          yield Approval$2().update(context.country, context.flag, context.store, context.draft.id, Object.assign(Object.assign({}, context.draft), {
            state: 'COMPLETED'
          }));
  ...
  const onRemoveConfirmActionClickHandler = action => {
    ...
    setTimeout(() => {
      if (action === 'remove') {
        ...
        const user = _.clone(draft.user);
        const roles = user.roles.filter(role => role !== (pendingToRemove === null || pendingToRemove === void 0 ? void 0 : pendingToRemove.id));
        user.roles = roles;
        setDraft(_.defaults({}, {
          user
        }, draft));
...
const Roles = () => {
  ...
  const onChangeSelectionHandler = options => {
    ...
    const roles = options.map(option => option.value);
    const user = _.clone(draft);
    user.roles = roles;
    setDraft(_.defaults({}, user, draft));
  };
  const onRemoveConfirmActionClickHandler = action => {
    setRemoveConfirmModal(false);
    setTimeout(() => {
      if (action === 'remove') {
        ...
        const roles = draft.roles.filter(role => role !== (pendingToRemove === null || pendingToRemove === void 0 ? void 0 : pendingToRemove.id));
        setDraft(_.defaults({}, {
          roles
        }, draft));
      }
  ...
  if (!draft) return jsx(Ramen.XPage, {});
  const selection = (draft.roles || []).map(role => {
    const match = location.state.roles.find(r => r.id === role);
    return {
      id: role,
      label: (match === null || match === void 0 ? void 0 : match.label) || role.toUpperCase()
  ...
  if (selection.length > 0) {
    content = jsx(Ramen.XList, {
      dataSource: _.orderBy(selection, ['label'], ['asc']),
      renderItem: option => {
        var _a;
        const disabled = ((_a = location.state.roles.find(r => r.id === option.id)) === null || _a === void 0 ? void 0 : _a.assignable) === false;
        return jsx("div", Object.assign({
...
class Module extends SDK.Lib.BaseModule {
  constructor(params) {
    ...
    this.approvalPath = '/iam/approval';
    this.Approval = () => jsx(Approval, {
      params: this.staticParams
    });
    this.usersPath = '/iam/users';
