...
const Login = props => {
  ...
  const onLoginHandler = provider => {
    const loginUrl = `${config.getValue('IAM_API_BASE_URL')}/oauth2/${CLIENT_ID}/authorize/${provider}?response_type=token&prompt=login&redirect_uri=${config.getValue('IAM_API_BASE_URL')}/oauth2/callback`;
