...
class PrintsPage extends Page {
  constructor() {
    ...
    this.onClickPrintHandler = print => {
      Ramen.Api.loading.show({
        text: 'Cargando...'
      });
      setTimeout(() => {
        Ramen.Api.loading.hide();
        this.props.history.push('/price-news/sections');
      }, 1000);
    };
  }
  render() {
    return jsxs(Ramen.XPage, {
      children: [jsx(Ramen.XHeader, {
        ...
        children: [jsx(Ramen.XText, Object.assign({
          ...
          dataSource: this.state.prints,
          renderItem: item => jsx(Ramen.XCard, {
            title: item.name,
            onClick: () => this.onClickPrintHandler(item)
          })
        })]
      })]
    });
